...
FILENAMES = {};
dobzinski_gerst_2015.pmid = 26344761;

% MANUAL. Download the list of dataset ids and standard names from
% the paper's page on www.yeastphenome.org & save the file to ./extras

% Load the list
[FILENAMES{end+1}, d] = read_data('textread', ['./extras/YeastPhenome_' num2str(dobzinski_gerst_2015.pmid) '_datasets_list.txt'],'%d %s','delimiter','\t');
datasets.id = d{1};
...
% Get the data itself
hit_data = data(3:end,4:5);
hit_data(strcmp('+', hit_data)) = {1};
hit_data(strcmp('±', hit_data)) = {0.5};
...
dobzinski_gerst_2015.orfs = tested_strains;
dobzinski_gerst_2015.ph = hit_data_names;
dobzinski_gerst_2015.data = zeros(length(dobzinski_gerst_2015.orfs),length(dobzinski_gerst_2015.ph));
dobzinski_gerst_2015.dataset_ids = hit_data_ids;

[~,ind1,ind2] = intersect(hit_strains, dobzinski_gerst_2015.orfs);
dobzinski_gerst_2015.data(ind2,:) = hit_data(ind1,:);

%% Save

save('./dobzinski_gerst_2015.mat','dobzinski_gerst_2015');

%% Print out

fid = fopen('./dobzinski_gerst_2015.txt','w');
write_matrix_file(fid, dobzinski_gerst_2015.orfs, dobzinski_gerst_2015.ph, dobzinski_gerst_2015.data);
fclose(fid);
...
if exist('save_data_to_db.m')
    res = save_data_to_db(dobzinski_gerst_2015)
end
